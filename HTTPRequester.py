...
class HTTPRequester:
    ...
    def argus_redirect(self, url, data, headers):
        try:
            ...
            response = requests.post(f"{self.API_URL}/get_sign", headers=self.API_HEADERS, data=payload)
    ...
    def _send_request(self, method, url, headers=None, data=None, allow_redirects=False, **kwargs):
        try:
            ...
            while response.is_redirect:
                ...
                generated_headers = self.argus_redirect(url, data if method == 'POST' and data else "", updated_headers)
